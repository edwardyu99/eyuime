...
parser.add_argument('--startsWith', '-s', type=str, default='a',
                    help='指定編碼前綴 (a-z)，預設 a')
args = parser.parse_args()

STARTSWITH = args.startsWith
if len(STARTSWITH) != 1 or not STARTSWITH.isalpha():
    print("錯誤：--startsWith 必須為單個字母 a-z")
    sys.exit(1)
STARTSWITH = STARTSWITH.lower()
# ============================================================
# 可配置參數：指定要處理的編碼前綴（例如 'f', 'g', 'a' 等）
## STARTSWITH = 'a'   # 可自行修改為其他字母，例如 'g'
...
if os.path.exists(final_out):
    ...
    with open(filedup, 'w', encoding='utf-16') as fd:
        ...
        fd.write(f"6. 雙字詞編碼數重碼率：{homonym_rate:.2f}%\n")
        fd.write("-" * 30 + "\n")
        fd.write("*** write >= f-dup lines only\n")
        print("*** write >= f-dup lines only")
        for code in dup_codes_list:
            if code >= "f     ":  #write dup lines after "f" lines, prev done
                # 修改点：使用 :<6 左对齐并补足到6个字符
                fd.write(f"{code:<6} {' '.join(code_map[code])}\n")
