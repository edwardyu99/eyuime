...
print(f'** yus_candict_head_scan_tonefreq.py scan {filepath} for tonefreq')
#cheung 張
...
with open(filepath,'r', encoding='utf-16') as fp:  
   ...
   while line:
     ...
     if tone_value == 'x':
     	...
     	dictcsv='cuhk_charfreq7000_粵拼_ori.csv'
     	print(f"{filepath} {line} key: {key_ch} not found in {dictcsv} , default tone = 1")
...
with open(input_file, 'r', encoding='utf-16') as fin, open(output_file, 'w', encoding='utf-16') as fout:
    ...
    for line in fin:
        ...
        if freq < '1000':
            oseq = 0
            tone = '1'
        if freq > '1500' and tone1 != '2':
        	  oseq = seq + 1
        #if seq == 0 and tone == 2:
        #    oseq = 0
        
        fout.write(line + ' ' + str(oseq).zfill(2) + str(tone) + '\n')
        #tone += 1
        #if tone > 9:
        #	tone = 9
        seq += 1
...
# Sort the lines
sorted_lines = sorted(open(input_file, 'r', encoding='utf-16'), key=lambda x: (x[0:6], x[16:19])) # x[17:20]))
...
# input_file = 'yus_candict_head_scan_tonegroupsort.txt' # just scan no need to sort
input_file = 'yus_candict_head_scan_tonegroup.txt'
