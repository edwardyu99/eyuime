import codecs
import sys
import os
from collections import Counter

def process_yucode(input_path):
    if not os.path.exists(input_path):
        print(f"錯誤：找不到檔案 {input_path}")
        return

    # 1. 建立聲調映射表 (從 jyutping.txt)
    tone_map = {}
    if os.path.exists('jyutping.txt'):
        with codecs.open('jyutping.txt', 'r', 'utf-8') as f:
            next(f)
            for line in f:
                parts = line.strip().split('\t')
                if len(parts) >= 6:
                    char, tone = parts[0], parts[5]
                    if char not in tone_map:
                        tone_map[char] = tone

    # 2. 建立排名映射表 (從 cuhk_ch_rank_7000i.txt)
    rank_map = {}
    if os.path.exists('cuhk_ch_rank_7000i.txt'):
        with codecs.open('cuhk_ch_rank_7000i.txt', 'r', 'utf-8') as f:
            next(f)
            for line in f:
                line = line.strip()
                if ',' in line:
                    char, rank = line.split(',')
                    rank_map[char] = rank.strip()

    # 定義 9 位聲調循環順序
    TONE_CYCLE = ["1", "2", "3", "4", "5", "6", "1", "2", "3"]

    def reorder_globally_by_cycle(all_items, target_cycle):
        """
        跨組抓取邏輯：優先填滿聲調循環。
        """
        reordered = []
        remaining_pool = list(all_items) # 已經按 rank 排序
        
        # 持續進行分組，直到 pool 空了
        while remaining_pool:
            # 決定當前這一組需要的循環長度 (最後一組可能不足 9)
            current_cycle = target_cycle[:min(len(remaining_pool), 9)]
            
            group_items = []
            for target_tone in current_cycle:
                # 從剩餘的 pool 中尋找「最常用」且「符合聲調」的字
                match = next((x for x in remaining_pool if x['tone'] == target_tone), None)
                
                if match:
                    group_items.append(match)
                    remaining_pool.remove(match)
                else:
                    # 如果全局都找不到該聲調（極罕見情況），才抓 pool 裡最常用的補位
                    if remaining_pool:
                        fallback = remaining_pool.pop(0)
                        group_items.append(fallback)
            
            reordered.extend(group_items)
            
        return reordered

    # 3. 讀取輸入檔案
    raw_data = []
    codes_counter = Counter()

    with codecs.open(input_path, 'r', 'utf-16') as f_in:
        for line in f_in:
            line = line.strip()
            if not line: continue
            
            parts = line.split()
            if len(parts) >= 2:
                code, char = parts[0], parts[1]
                tone = tone_map.get(char, "9")
                rank = rank_map.get(char, "9999")
                
                raw_data.append({
                    'code': code,
                    'char': char,
                    'tone': tone,
                    'rank': int(rank)
                })
                codes_counter[code] += 1

    # 4. 執行「跨組」循環排序邏輯
    final_sorted_list = []
    unique_codes = sorted(codes_counter.keys())

    for c in unique_codes:
        # 該編碼所有字，先按全局字頻排序
        full_group = sorted([item for item in raw_data if item['code'] == c], key=lambda x: x['rank'])
        
        # 呼叫新邏輯：全局掃描聲調來填坑
        reordered_full_group = reorder_globally_by_cycle(full_group, TONE_CYCLE)
        final_sorted_list.extend(reordered_full_group)

    # 5. 輸出檔案
    base_name = os.path.splitext(input_path)[0]
    output_file = f"{base_name}_special.txt"
    output_file2 = f"{base_name}_special2.txt"

    with codecs.open(output_file, 'w', 'utf-16') as f_out, \
         codecs.open(output_file2, 'w', 'utf-16') as f_out2:
        for item in final_sorted_list:
            f_out.write(f"{item['code']:<6} {item['char']} {item['tone']} {item['rank']:04d}\n")
            f_out2.write(f"{item['code']:<6} {item['char']}\n")

    print(f"「跨組抓取」循環排序完成！")
    print(f"邏輯：優先從全局提取符合 1,2,3,4,5,6,1,2,3 順序的字。")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法：python yus_sort_code_special.py yus_candict_c.txt")
    else:
        process_yucode(sys.argv[1])