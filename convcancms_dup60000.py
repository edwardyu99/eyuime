...
# ==================== 新增：命令行参数解析 ====================
parser = argparse.ArgumentParser(description='處理粵語拼音碼表')
parser.add_argument('--startsWith', '-s', type=str, default='ab',
                    help='指定編碼前綴區間 (a:z)，預設 ab')
args = parser.parse_args()

STARTSWITH = args.startsWith
if len(STARTSWITH) != 2 or not STARTSWITH.isalpha():
    print("錯誤：--startsWith 必須為雙字母 a:z")
    sys.exit(1)
STARTSWITH = STARTSWITH.lower()
# ============================================================
# 可配置參數：指定要處理的編碼前綴區間（例如 'aa', 'ab'）
...
filekeep = f'reneeyu_canph2345_keep{STARTSWITH}.txt'
...
# =====================================================================
# 第四階段：雙字詞重碼分析（含人工篩選 f 開頭編碼，逐詞輸出）
# =====================================================================
print("\n--- 開始分析雙字詞重碼情況 ---")
...
if os.path.exists(final_out):
    ...
    with open(filedup, 'w', encoding='utf-16') as fd:
        ...
        fd.write("-" * 30 + "\n")
        fd.write("*** 所有重碼編碼及對應詞組（未篩選）\n")
        for code in dup_codes_list:
            fd.write(f"{code:<6} {' '.join(code_map[code])}\n")

    # 2. 收集指定前綴區間開頭的雙字詞（包括重碼與不重碼）
    f_words_to_keep = []
    
    # 解析區間起訖字母
    start_char = STARTSWITH[0]
    end_char = STARTSWITH[1]
    
    # 篩選首字母落在 start_char 與 end_char 之間的編碼
    target_codes = [code for code in code_map.keys() if code and start_char <= code[0] <= end_char]
    
    print(f"\n共發現 {len(target_codes)} 個以 '{start_char}' 到 '{end_char}' 區間開頭的編碼，開始處理...")

    for code in target_codes:
        words = code_map[code]
        if len(words) == 1:
            # 不重碼：直接保留
            f_words_to_keep.append(f"{code:<6} {words[0]}\n")
            # print(f"  {code} (單詞) 自動保留：{words[0]}")
        
        else:
            # 重碼：第1詞自動保留，第2詞起詢問
            print(f"\n編碼：{code} (共 {len(words)} 個重碼詞)")
            for idx, w in enumerate(words, start=1):
                if idx == 1:
                    f_words_to_keep.append(f"{code:<6} {w}\n")
                    print(f"  第1重碼: {code} {w} (自動保留)")
                    continue

                #--- 自動保留詞頻(RANK)少於60000的雙字詞，不用詢問
                word_rank = int(mergedict.get(w, 108888))
                if word_rank < 60000:
                    f_words_to_keep.append(f"{code:<6} {w}\n")
                    print(f"    ✓ 自動保留 (Rank: {word_rank})：{code} {w}")
                    continue

                while True:
                    prompt = f"  第{idx}重碼: {code} {w} (Rank: {word_rank}) 保留嗎？(y/n，預設 y): "
    ...
    # 3. 寫入保留結果檔案（所有位於指定區間開頭的雙字詞，每個詞單獨一行）
    if f_words_to_keep:
        with open(filekeep, 'w', encoding='utf-16') as fk:
            fk.write(f"【所有以 '{start_char}' 到 '{end_char}' 區間開頭雙字詞篩選結果】\n")
            fk.write("格式：編碼（6位左對齊） 詞\n")
            fk.writelines(f_words_to_keep)
        print(f"\n處理完成：已將 {len(f_words_to_keep)} 個 [{start_char}-{end_char}] 區間雙字詞保存至 {filekeep}")
    else:
        print(f"\n未發現任何以 [{start_char}-{end_char}] 區間開頭的雙字詞，未生成 keep 檔案。")

    '''
    # 2. 收集所有指定前綴開頭的雙字詞（包括重碼與不重碼）
    f_words_to_keep = []
    target_codes = [code for code in code_map.keys() if code.startswith(STARTSWITH)]
    print(f"\n共發現 {len(target_codes)} 個以 '{STARTSWITH}' 開頭的編碼，開始處理...")
    ...
    for code in target_codes:
        ...
        if len(words) == 1:
            ...
            for idx, w in enumerate(words, start=1):
                ...
                while True:
                    # 在提示字串中加入 (Rank: {word_rank})
                    #prompt = f"  第{idx}重碼: {code} {w} 保留嗎？(y/n，預設 y): "
                    prompt = f"  第{idx}重碼: {code} {w} (Rank: {word_rank}) 保留嗎？(y/n，預設 y): "
                    resp = input(prompt).strip().lower()
                ...
                if keep:
                    ...
                    print(f"    ✗ 已丟棄：{code} {w}")
            #print()  # 讓控制台輸出分隔，編碼組之間空一行
        
    # 3. 寫入保留結果檔案（所有 f 開頭的雙字詞，每個詞單獨一行）
    if f_words_to_keep:
        with open(filekeep, 'w', encoding='utf-16') as fk:
            fk.write(f"【所有以 '{STARTSWITH}' 開頭雙字詞篩選結果】\n")
            fk.write("格式：編碼（6位左對齊） 詞\n")
            fk.writelines(f_words_to_keep)
        print(f"\n處理完成：已將 {len(f_words_to_keep)} 個 f 開頭雙字詞保存至 {filekeep}")
    else:
        print("\n未發現任何 f 開頭的雙字詞，未生成 keep 檔案。")
    '''
    ...
    print(f"錯誤：找不到最終碼表 {final_out}")# 第五階段（其餘程式碼保持不變）
